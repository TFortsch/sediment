...
# balule = balule.reproject(crs='EPSG:32736', scale=1) # this didn't run: AttributeError: 'Geometry' object has no attribute 'reproject'.  BUT, not needed.
...
# mosaic.normalizedDifference(['B3', 'B8']) # Gao
# NDWI = ( NIR - SWIR ) / ( NIR + SWIR )
ndwiM = (b8-b11)/(b8_b11) # McFeeters
# mosaic.normalizedDifference(['B8', 'B11']) # McFeeters
# NDWI = ( G - SWIR ) / ( G + SWIR )
mndwi = (b3-b11)/(b3+b11) # Modified NDWI
# mosaic.normalizedDifference(['B3', 'B11']) # Modified NDWI

# Dekker 2002
dekker = (b3+b4)/2
# ((mosaic.select('B3')).add(mosaic.select('B4'))).divide(2)
water = ndwiG > -0.02
